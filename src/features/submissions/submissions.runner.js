...
class PythonRunner {
  ...
  _handleLine(line) {
    ...
    try {
      ...
      logger.warn({ line: trimmed.slice(0, 120) }, "Invalid python runner payload");
      return;
    }
...
async function runWithSpawn({ language, code }) {
  ...
  const tempFile = path.join(os.tmpdir(), `riwlog-run-${randomUUID()}.${runner.ext}`);
  const start = Date.now();
  ...
  try {
    const child = spawn(runner.command, [...runner.args, tempFile], {
    ...
    const { code: exitCodeValue, signal } = await exitPromise;
    clearTimeout(timeout);
    exitCode = exitCodeValue ?? (signal ? -1 : null);
  } catch (error) {
    logger.error({ err: error, language: normalizedLanguage }, "Failed to run user code");
    stderrCollector.push(`
No se pudo ejecutar el codigo (${error?.message || "error inesperado"}).`);
  } finally {
    fs.unlink(tempFile).catch(() => {});
  }

  const stdout = truncateOutput(Buffer.concat(stdoutCollector).toString("utf8"));
  let stderr = truncateOutput(Buffer.concat(stderrCollector).toString("utf8"));
  if (timedOut) {
    stderr = `${stderr ? `${stderr}
` : ""}Tiempo de ejecucion excedido (${TIMEOUT_MS}ms).`;
  }
